...
if upload_df is not None:
    ...
    with col1:
        ...
        group_column = st.radio("Select the group column", df.columns, index=None)
    ...
    with col2:
        ...
        alpha = st.slider('Significance Level for [T-Test](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_ind.html) / [Mann Whitney U Test](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.mannwhitneyu.html)', 0.0, 1.0 , 0.5,0.1)
    ...
    if len(groups_to_compare) == 2 and len(metric_columns) >0:
        ...
        if st.button('Plots Plots Plots'):
            for column in metric_columns:
                    ...
                    fig.add_trace(go.Histogram(
                        x=dataset1[column],
                        #histnorm='percent',
                        name=f'{groups_to_compare[0]}:{column}',
                        marker_color='#1f77b4',
                        opacity=0.75
                    ))
                    fig.add_trace(go.Histogram(
                        x=dataset2[column],
                        #histnorm='percent',
                        name=f'{groups_to_compare[1]}:{column}',
                        marker_color='#d62728',
                        opacity=0.75
                    ))

                    fig.update_layout(
                        title_text=f'{column}', # title of plot
        ...
        if len(metric_columns) == 0:
            st.markdown(':warning: :red[Please choose at least one metric]')
